...
#include "plug_ogg.h"
DATA_IN_AfterSystem static bool isOGG=false;

static bool DLLSound_ChkExt_internal_ogg(const char *pext)
{
  bool res=false;
  if(isStrEqual_NoCaseSensitive(pext,".ogg")==true) res=true;
  return(res);
}

static bool DLLSound_Open_internal_ogg(const char *pext)
{
    OVM_libsnd_ogg();
  if(PlugOGG_Start(PluginBody_FileHandle)==false){
    PlugOGG_Free();
    FAT2_fclose(PluginBody_FileHandle); PluginBody_FileHandle=NULL;
    _consolePrint("Can not start internal OGG plugin.\n");
    return(false);
  }
  DTCM_StackCheck(-1);
  
  u32 rate=PlugOGG_GetSampleRate();
  u32 spf=PlugOGG_GetSamplePerFrame();
  u32 chs=PlugOGG_GetChannelCount();
  
  MACRO_LoadGeneralID3Tag(&MM_DLLSound,PlugOGG_GetInfoIndexCount,PlugOGG_GetInfoStrL,PlugOGG_GetInfoStrUTF8,PlugOGG_GetInfoStrW);
...
static void DLLSound_Update_internal_ogg(u32 BaseSamples,u32 *plrdst)
{
    u32 Samples=0;
    if(RateConv.SourceRate==0){
        ...
        {
          // PrfStart();
          u32 lastpos=PlugOGG_GetPosOffset();
          Samples=PlugOGG_Update(plrdst);
          DTCM_StackCheck(-1);
          u32 curpos=PlugOGG_GetPosOffset();
      ...
      
      {
        ...
        while((((s32)prc->Size-1)-(s32)prc->Pos)<(s32)prc->CountSource){
          { cwl();
            // PrfStart();
            u32 lastpos=PlugOGG_GetPosOffset();
            u32 Samples=PlugOGG_Update(&prc->pReadBufLR[prc->Size]);
            DTCM_StackCheck(-1);
            u32 curpos=PlugOGG_GetPosOffset();
            if((PlugTime_SkipCheck==false)&&(lastpos<=curpos)){
              PlugTime_TotalSamples+=Samples;
...
static u32 DLLSound_GetPlayTimeSec_internal_ogg(void)
{
  return(PlugOGG_GetPlayTimeSec());
}
