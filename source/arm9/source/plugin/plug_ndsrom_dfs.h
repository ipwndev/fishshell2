...
static void CreateBurstList(FAT_FILE *pFileHandle)
{
  ...
  pBurstList=(TBurstList*)malloc(BurstListCount*sizeof(TBurstList));
...
static void FreeBurstList(void)
{
  BurstListCount=0;
  if(pBurstList!=NULL){
    free(pBurstList); pBurstList=NULL;
  }
}
...
static void MoveTop(TDFS_File *pf)
{
  pf->Offset=0;
...
static void ReadSkip(TDFS_File *pf,u32 size)
{
...
static u32 Read16bit(TDFS_File *pf,u8 *pbuf,u32 size)
{
...
static void DFS_SetOffset(u32 ofs)
{
...
static u32 DFS_Read16bit(void *_pbuf,u32 size)
{
  TDFS_File *pf=&DFS_File;
