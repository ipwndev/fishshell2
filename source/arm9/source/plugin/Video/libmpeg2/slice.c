...
extern void mpeg2_idct_add(int last, int16_t * block, uint8_t * dest, int stride);
...
static CODE_IN_ITCM_DPG int get_motion_delta (mpeg2_decoder_t * const decoder,
                  const int f_code)
{
#define bit_buf (decoder->bitstream_buf)
#define bits (decoder->bitstream_bits)
...
static CODE_IN_ITCM_DPG void get_mpeg1_intra_block (mpeg2_decoder_t * const decoder)
{
  int i;
  int j;
  int val;
...
static CODE_IN_ITCM_DPG void motion_mp1 (mpeg2_decoder_t * const decoder,
...
#define MOTION_FUNCTIONS2_1(FORMAT,MOTION,MOTION_FIELD,MOTION_DMV,MOTION_ZERO)   \
static CODE_IN_ITCM_DPG void motion_reuse_##FORMAT (mpeg2_decoder_t * const decoder,        \
                   motion_t * const motion,            \
                   mpeg2_mc_fct * const * const table)        \
{\
  int motion_x, motion_y;                            \
...
MOTION_FUNCTIONS2_1 (420, MOTION_420, MOTION_FIELD_420, MOTION_DMV_420, MOTION_ZERO_420)

#define MOTION_FUNCTIONS2_2(FORMAT,MOTION,MOTION_FIELD,MOTION_DMV,MOTION_ZERO)   \
static CODE_IN_ITCM_DPG void motion_zero_##FORMAT (mpeg2_decoder_t * const decoder,        \
                  motion_t * const motion,            \
                  mpeg2_mc_fct * const * const table)        \
{\
  unsigned int offset;                            \
...
MOTION_FUNCTIONS2_2 (420, MOTION_420, MOTION_FIELD_420, MOTION_DMV_420, MOTION_ZERO_420)
...
bool mpeg2_slice_start (mpeg2_decoder_t * const decoder, const int code, const uint8_t * const buffer)
{
  bitstream_init (decoder, buffer);

  if (slice_init (decoder, code)) return(false);
  
  return(true);
}

static void mpeg2_slice_intra(mpeg2_decoder_t * const decoder, const int code, const uint8_t * const buffer, int macroblock_modes)
{
#define bit_buf (decoder->bitstream_buf)
#define bits (decoder->bitstream_bits)
#define bit_ptr (decoder->bitstream_ptr)
      ...
      if (likely (decoder->chroma_format == 0)) {
        ...
        if (likely (decoder->chroma_format == 1)) {
...
const char errmsg_macroblock_modes_MACROBLOCK_QUANT_not_implimented[]="macroblock_modes & MACROBLOCK_QUANT not implimented.\n";

void CODE_IN_ITCM_DPG mpeg2_slice (mpeg2_decoder_t * const decoder, const int code, const uint8_t * const buffer)
{
#define bit_buf (decoder->bitstream_buf)
#define bits (decoder->bitstream_bits)
#define bit_ptr (decoder->bitstream_ptr)
...
//      StopFatalError(16001,errmsg_macroblock_modes_MACROBLOCK_QUANT_not_implimented);
//      get_quantizer_scale (decoder);
    }

    if (macroblock_modes & MACROBLOCK_INTRA) {
      mpeg2_slice_intra(decoder,code,buffer,macroblock_modes);
      ...
      if (macroblock_modes & MACROBLOCK_PATTERN) {
        ...
        if (likely (decoder->chroma_format == 0)) {
          ...
          if (coded_block_pattern & 1) slice_non_intra_DCT (decoder, 0, dest_y, DCT_stride);
          if (coded_block_pattern & 2) slice_non_intra_DCT (decoder, 0, dest_y + 8, DCT_stride);
          if (coded_block_pattern & 4) slice_non_intra_DCT (decoder, 0, dest_y + DCT_offset, DCT_stride);
          if (coded_block_pattern & 8) slice_non_intra_DCT (decoder, 0, dest_y + DCT_offset + 8, DCT_stride);
          if (coded_block_pattern & 16) slice_non_intra_DCT (decoder, 1, decoder->dest[1] + (offset >> 1), decoder->uv_stride);
          if (coded_block_pattern & 32) slice_non_intra_DCT (decoder, 2, decoder->dest[2] + (offset >> 1), decoder->uv_stride);
          }else{
          if (likely (decoder->chroma_format == 1)) {
            int offset;
            uint8_t * dest_y;

            coded_block_pattern |= bit_buf & ((u32)3 << 30);
            DUMPBITS (bit_buf, bits, 2);
            ...
            if (coded_block_pattern & 16) slice_non_intra_DCT (decoder, 1, decoder->dest[1] + (offset >> 1), DCT_stride);
            if (coded_block_pattern & 32) slice_non_intra_DCT (decoder, 2, decoder->dest[2] + (offset >> 1), DCT_stride);
            if (coded_block_pattern & ((u32)2 << 30)) slice_non_intra_DCT (decoder, 1, decoder->dest[1] + DCT_offset, DCT_stride);
            if (coded_block_pattern & (1 << 30)) slice_non_intra_DCT (decoder, 2, decoder->dest[2] + DCT_offset, DCT_stride);
            }else{
            int offset;
            uint8_t * dest_y, * dest_u, * dest_v;
  
            coded_block_pattern |= bit_buf & ((u32)63 << 26);
            ...
            if (coded_block_pattern & 16) slice_non_intra_DCT (decoder, 1, dest_u, DCT_stride);
            if (coded_block_pattern & 32) slice_non_intra_DCT (decoder, 2, dest_v, DCT_stride);
            if (coded_block_pattern & ((u32)32 << 26)) slice_non_intra_DCT (decoder, 1, dest_u + DCT_offset, DCT_stride);
            if (coded_block_pattern & (16 << 26)) slice_non_intra_DCT (decoder, 2, dest_v + DCT_offset, DCT_stride);
            if (coded_block_pattern & (8 << 26)) slice_non_intra_DCT (decoder, 1, dest_u + 8, DCT_stride);
            if (coded_block_pattern & (4 << 26)) slice_non_intra_DCT (decoder, 2, dest_v + 8, DCT_stride);
            if (coded_block_pattern & (2 << 26)) slice_non_intra_DCT (decoder, 1, dest_u + DCT_offset + 8, DCT_stride);
            if (coded_block_pattern & (1 << 26)) slice_non_intra_DCT (decoder, 2, dest_v + DCT_offset + 8, DCT_stride);
          }
        }
      }
