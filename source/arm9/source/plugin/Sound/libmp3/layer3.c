			    ...
			    unsigned int *priv_bitlen)
{
  ...
  *priv_bitlen = lsf ? ((nch == 1) ? 1 : 2) : ((nch == 1) ? 5 : 3);
  ...
  for (gr = 0; gr < ngr; ++gr) {
    Tsideinfo_granule *granule = (Tsideinfo_granule*)&si->gr[gr];

    for (ch = 0; ch < nch; ++ch) {
      Tsideinfo_channel *channel = (Tsideinfo_channel*)&granule->ch[ch];
...
unsigned int III_scalefactors_lsf(struct mad_bitptr *ptr,
				  Tsideinfo_channel *channel,
				  Tsideinfo_channel *gr1ch, int mode_extension)
{
 ...
 */
static
unsigned int III_scalefactors(struct mad_bitptr *ptr, Tsideinfo_channel *channel,
			      Tsideinfo_channel const *gr0ch, unsigned int scfsi)
{
  struct mad_bitptr start;
 ...
 */
static
void III_exponents(Tsideinfo_channel const *channel,
 ...
 */
static
void III_reorder(mad_fixed_t xr[576], Tsideinfo_channel const *channel,
...
enum mad_error III_stereo(mad_fixed_t xr[2][576],
			  Tsideinfo_granule const *granule,
			  ...
			  unsigned char const *sfbwidth)
{
  ...
  if (header->mode_extension & I_STEREO) {
    Tsideinfo_channel const *right_ch = &granule->ch[1];
...
# if defined(ASO_IMDCT)
extern "C" void III_imdct_l(mad_fixed_t const [18], mad_fixed_t [36], unsigned int);
# else
#  if 1
static
			  ...
			  struct sideinfo *si, unsigned int nch)
{
  ...
  for (gr = 0; gr < ngr; ++gr) {
    Tsideinfo_granule *granule = &si->gr[gr];
    ...
    for (ch = 0; ch < nch; ++ch) {
      Tsideinfo_channel *channel = &granule->ch[ch];
      unsigned int part2_length;
    ...
    for (ch = 0; ch < nch; ++ch) {
      Tsideinfo_channel const *channel = &granule->ch[ch];
