...
DATA_IN_IWRAM_FileList static u8 *pTextPoolStart=NULL;
DATA_IN_IWRAM_FileList static u8 *pTextPoolEnd;
DATA_IN_IWRAM_FileList static u8 *pTextPoolPos;

static void FreeTextPool(void)
{
  if(pTextPoolStart!=NULL){
    safefree(&MM_Process,pTextPoolStart); pTextPoolStart=NULL;
  }
}
...
static void InitTextPool(void)
{
  ...
  pTextPoolStart=(u8*)safemalloc_chkmem(&MM_Process,maxsize);
...
static void EndTextPool(void)
{
  ...
  u8 *ptmp=(u8*)saferealloc(&MM_Process,pTextPoolStart,pTextPoolPos-pTextPoolStart);
  
  if(pTextPoolStart!=ptmp) StopFatalError(17102,"TextPool saferealloc error. 0x%x!=0x%x\n",pTextPoolStart,ptmp);
...
static u8* GetTextPool(u32 size)
{
  ...
  if(pTextPoolEnd<=(p+size)) StopFatalError(17103,"TextPool memory overflow. size=%dbyte\n",size);
